...
rest.PP <- lapply(1:nperms, function(P) cor.test(null_energy[P,],rtp[onDiag]))
rest.PP.r <- sapply(rest.PP,function(x) x$estimate)
rest.PP.sig <- as.numeric(sapply(rest.PP,function(x) x$p.value < 0.05))
nback.PP <- lapply(1:nperms, function(P) cor.test(null_energy[P,],ntp[onDiag]))
...
actual <- lapply(list(rtp[onDiag],ntp[onDiag]),function(x) cor.test(x,as.numeric(energy)))
...
pvals <- p.adjust(c(mean(actual[1] > rest.PP.r),mean(actual[2] < nback.PP.r)),method = 'bonf')
