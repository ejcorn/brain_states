...
savedir = fullfile(basedir,'results/',...
    name_root,'analyses','control_energy','model_task_inputs',['DLWNull_c',num2str(c),'T',num2str(T)]);
...
for P = perm_range
  ...
  for transition = 1:num_transitions    
       [x, u, NumericalError(transition)] = MIN_ENG_CONT(A_DLWnullnorm, T, B, x0(:,transition), xf(:,transition), 0);       
       DistanceToTarget(transition) = sum((xf(:,transition)-x(end,:)').^2);
       E_full(transition) = sum(sum(u.^2))*T/1000;
  end
  save(fullfile(savedir,['DLWNullPerm',num2str(P),'_FA_TransitionEnergies',SystemLabel,ControlLabel,'Control_c',...
  num2str(c),'T',num2str(T),'_k',num2str(numClusters),'.mat']),'E_full','DistanceToTarget','NumericalError');
end