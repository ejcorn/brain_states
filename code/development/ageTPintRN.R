...
# does association between age and transition probability interact with scan type?
...
df <- rbind(cbind(restTP,demo,RN=rep(0,nrow(demo))),cbind(nbackTP,demo,RN=rep(1,nrow(demo))))

int.age <- lapply(1:numTransitions, function(T) summary(lm.beta(lm(df[,as.character(T)] ~ age_in_yrs*RN + BrainSegVol + handedness + restRelMeanRMSMotion + Sex, 
	data = df))))
names(int.age) <- transLabels
print(int.age['DMN- to FPN+'])
print(int.age['DMN+ to FPN+'])

# extract interaction coefficient
int.TPage <- lapply(int.age, function(X) X$coefficients[8,c('Standardized','Pr(>|t|)')])

pdat <- cbind(as.data.frame(t(as.data.frame(int.TPage, row.names = c('intB','intP')))))
rownames(pdat) <- transLabels
p.list <- list.posthoc.correct(list(pdat$intP),method = 'bonf')  # bonferroni correct
print(p.list)
pdat$intP <- p.list[[1]] #< 0.05

b.mat <- matrix(pdat$intB, nrow = numClusters, byrow = TRUE)
p.mat <- matrix(pdat$intP, nrow = numClusters, byrow = TRUE)
p <- plot.beta.matrix(b.mat,p.mat,clusterColors,clusterNames,title = 'Rest-by-Task')
ggsave(plot = p,filename = paste(savedir,'RestnBackInteractionWithAgeOnTP_k',numClusters,'.pdf',sep =''),units = 'cm',height = 6,width = 6)
