...
restDur <- readMat(paste(masterdir,'analyses/transitionprobabilities/RestCombDwellTime_k',numClusters,name_root,'.mat',sep = ''))$stateDuration * 100
nbackDur <- readMat(paste(masterdir,'analyses/transitionprobabilities/nBackCombDwellTime_k',numClusters,name_root,'.mat',sep = ''))$stateDuration * 100
...
ggsave(plot = p, filename = paste(masterdir,'analyses/fractionaloccupancy/RvNDwellTime_k',numClusters,'.pdf',sep =""),height = 2,width = (numClusters-1) + 0.25, units = "in")