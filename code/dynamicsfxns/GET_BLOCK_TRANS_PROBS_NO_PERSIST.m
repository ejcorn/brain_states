function [transitionProbabilityMat,transitionProbabilityVec] = GET_BLOCK_TRANS_PROBS_NO_PERSIST(partition,indicator,numClusters)
...
partition = reshape(partition,1,[]); % force into row vector
...
for j = 1:n_runs
	numTransitionsByBlock{j} = GET_NUM_TRANSITIONS_NO_PERSIST(runs{j},numClusters);
end
